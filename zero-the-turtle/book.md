![SPIFFE Book Cover](assets/Image_001.jpg)
...
![Book Sprint Participants](assets/Image_005.jpg)
...
*Fig. 1.1: Evolution of networks from the conﬁnes of the university campus to
...
![Traditional architecture vs modern chaos.](assets/Image_016.jpg)
...
![Everyone needs access to everything all the time.](assets/Image_018.jpg)
...
*Fig. 2.1: Meeting compliance and regulatory objectives seamlessly using 
SPIFFE.*
...
The most signiﬁcant difference is the source. With passports, we trust that the 
...
*Fig. 3.1: Illustration of Bob requesting a certiﬁcate from the Certiﬁcate 
...
This leads to a key weakness of X.509: *any CA can sign any certiﬁcate, with no 
...
![SPIFFE Federation](assets/Image_077.jpg)
...
*Fig. 4.6: Illustration of the key plugin interfaces supported by SPIRE. The 
...
![Node attestation.](assets/Image_085.jpg)
...
A more long-term beneﬁt is that SPIRE can increase the overall security posture 
...
*Fig. 5.3: Adding gateways to a non-SPIFFE island is a way to bridge SPIFFE and 
non-SPIFFE islands.*
...
Choosing when a tool is good enough for the ﬁrst and early adopters is a 
...
workload identiﬁer section (the path portion of the URI) appended to the trust 
...
You may ﬁnd it useful to identify a service directly by the functionality it 
...
A single SPIRE server, in a high availability conﬁguration, is the best starting 
point for environments with a single trust domain.

![Single trust domain](assets/Image_143.jpg)

*Fig. 6.1: Single trust domain.*
...
to conﬁgure SPIRE Servers in a nested topology.
...
In this conﬁguration, the top-tier SPIRE Servers hold the root certiﬁcates and 
keys, and the downstream servers request an intermediate signing certiﬁcate to 
...
![Nested SPIRE.](assets/Image_145.jpg)

*Fig. 6.2: Nested SPIRE topology.*
...
![Nested topology.](assets/Image_146.jpg)

*Fig. 6.3: Illustration of a company architecture with one upstream SPIRE Server 
and two nested SPIRE Servers. Each of the two nested SPIRE Servers can have its 
own conﬁguration (relevant for AWS and Azure), and if either one of them fails, 
...
![Federated SPIRE.](assets/Image_148.jpg)

*Fig. 6.4: SPIRE Server using Federated trust domains.*

These multiple trust domains and interoperability use cases both require a 
well-deﬁned, interoperable method for a workload in one trust domain to 
...
![Single SPIRE.](assets/Image_150.jpg)

*Fig. 6.5: A single dedicated SPIRE Server.*

##### Avoiding a single point of failure

The beneﬁt of simplicity always comes with a trade-off. If there is only one 
...
To scale the SPIRE Server horizontally, conﬁgure all servers in the same trust 
...
![Ha deployment.](assets/Image_151.jpg)

*Fig. 6.6: Multiple SPIRE Server instances running on HA.*
...
![Agent migration.](assets/Image_154.jpg)

*Fig. 6.7: What happens if you need to migrate all the agents in one cluster to 
another cluster?*
...
depending on the speciﬁc infrastructure. [Please check the SPIRE documentation 
for more details][spire-docs-datastore].

![Shared datastore.](assets/Image_156.jpg)

*Fig. 6.8: Two clusters using a Global data store scheme.*
...
![SPIRE Server on the same cluster.](assets/Image_163.jpg)

*Fig. 6.9: SPIRE Server on the same cluster as workloads.*

![SPIRE Server on dedicated cluster.](assets/Image_164.jpg)

*Fig. 6.10: SPIRE Server on a separate cluster for security.*
...
![Sidecar deploymennt.](assets/Image_168.jpg)

*Fig. 6.11: Workloads in a k8s cluster deployed along with side-car containers.*
...
![Node attestation.](assets/Image_172.jpg)

*Fig. 6.12: Node Attestor architecture and flow.*
...
![Manual workload registration.](assets/Image_174.jpg)

*Fig. 6.13: Workload manual registration.*
...
![Registration automation.](assets/Image_175.jpg)

*Fig. 6.14: An example of automatically creating workload registration entries 
using an “Identity Operator” that communicates with the Workload Orchestrator.*
...
![Trust chain.](assets/Image_178.jpg)

*Fig. 6.15: An example SPIRE deployment with three trust domains, each of which 
uses a different corporate Certiﬁcate Authority, each of which uses the same 
Root Certiﬁcate Authority. In each layer, certiﬁcates have a shorter TTL.*

Each organization will have a different certiﬁcate hierarchy because each 
...
When dealing with PKI, questions around certiﬁcate expiry, re-issuance, and 
...
signs its intermediate CA certiﬁcate by communicating with existing PKI using 
...
database, and co-hosting it with the server signiﬁcantly limits the attack 
surface, it is very hard to scale in a production environment.

![Remote data store.](assets/Image_180.jpg)

*Fig. 6.16: Typically the SPIRE Server data store runs on remotely over a 
...
dependencies for the SPIRE Server, including your conﬁguration management or 
deployment software.

#### SPIRE Agent conﬁguration and trust bundle
...
The SPIRE Agent conﬁguration also needs to be kept secure. If an attacker can 
...
model doesn’t consider that misconﬁgured (or intentionally conﬁgured) health 
...
It is the preferred choice when the modiﬁcations required are minimal, or can 
...
third-party code that can not be modiﬁed. In these situations, it is often a 
...
retrieve and maintain certiﬁcates and private keys.

![mTLS with Envoy.](assets/Image_191.jpg)

*Fig. 7.1: A high-level diagram of two Envoy proxies sitting between two 
services using the SPIRE Agent SDS implementation to establish mutual TLS.*

By implementing the SDS API, SPIRE can push TLS certiﬁcates, private keys, and 
trusted CA certiﬁcates directly into Envoy. SPIRE then takes care of rotating 
the short-lived keys and certiﬁcates as required, pushing updates to Envoy 
...
are written in many different languages, or are not able to be modiﬁed). This 
...
implemented product-speciﬁc SPIFFE identity providers.
...
occur, the helper can signal the application (or run a conﬁgurable command) such 
that the changes are picked up by the running application.

Many off-the-shelf applications that support TLS can be conﬁgured to use SPIFFE 
this way. The SPIFFE Helper repository has examples of conﬁguring MySQL and 
...
configured to utilize certiﬁcates on disk.
...
configuration. For example, when using SPIFFE Helper to conﬁgure Apache HTTPD 
for mutual TLS, it is not possible to conﬁgure `mod_ssl` to only accept clients 
...
extension. SPIRE supports issuing X.509 certiﬁcates with speciﬁc CN and DNS SAN 
values that can be speciﬁed on a workload-by-workload basis (as part of a 
registration entry).
...
Services (AWS) Identity and Access Management (IAM). By conﬁguring IAM in an 
...
in nearly real-time. Certificate Transparency allows detecting certiﬁcates that 
have been maliciously acquired from a compromised certificate authority. It also 
makes it possible to identify certiﬁcate authorities that have gone rogue and 
are maliciously issuing certiﬁcates. To learn more about Certiﬁcate 
...
Another method you might consider is to enforce Certiﬁcate Transparency across 
...
in the certiﬁcate transparency server.
...
code. Veriﬁcation of the provenance of software artifacts and the set of steps 
...
desirable for SPIFFE veriﬁable identities (SVIDs) to be given to users. This 
...
can conﬁgure your SSO provider to use a SPIFFE ID for the sub claim, you might 
...
you do have the means to get a veriﬁable identity token, you could instead 
utilize a custom SPIRE attestor that accepts an identity token from your 
provider as a rudimentary means of attestation.

![OIDC exchange.](assets/Image_203.jpg)

*Fig. 7.2: An example of using an OIDC ID token for SPIRE authentication.*
...
OpenSSH supports authentication with Certiﬁcate Authorities (CAs) and
...
creating SSH certiﬁcates using SVIDs as authentication. This allows you to 
...
this certiﬁcate, users no longer need to have their SSH connection interrupted 
with a question about trusting a server’s host key on the first connection.

![SVIDs and SSH certificates.](assets/Image_204.jpg)

*Fig. 7.3: Using SVIDs to bootstrap SSH certiﬁcates.*
...
A post-authentication ﬁlter for requests on the secondary port should provide a 
...
This way, the underlying application is agnostic to the speciﬁc authentication 
...
> This chapter explains how to implement authorization policies that use SPIFFE 
> identities.
...
a limited number of identities that don’t change, it is easy to deﬁne and update 
...
The SPIFFE speciﬁcation doesn’t specify or limit what information you can or 
...
deﬁne what each part of the identity means. You can design your scheme in the 
format where you encode information by the order. In this case, the ﬁrst part 
might represent a region, the second environment, and so on.

```txt
spiffe://trust.domain.org/<region>/<dev,stage,prod>/<organization>/<workload_name>
```

Below is an example of the scheme and identity:

![SPIFFE ID.](assets/Image_220.jpg)

*Fig. 8.1: Components of a SPIFFE ID and potential meanings at one organization.*

An identity scheme can not only take the shape of a series of ﬁxed ﬁelds but can 
...
an example. You may notice that the ﬁelds not only refer to different attributes 
...
of the identity. For example, an identity with the preﬁx 
...
![SPIFFE ID scheme.](assets/Image_221.jpg)

Fig. 8.2: Illustration of another potential SPIFFE ID scheme.
...
by looking at the preﬁx of the identity and determining how to evaluate the 
suffix that follows. However, we note that because of this ordering, it is 
difficult to easily add new ﬁelds to the scheme.
...
extend ﬁelds into an identity scheme without much change. For example, you can 
...
The version could be the ﬁrst item and the most critical part of your scheme. 
...
authorization simple, it’s best to ﬁrst have one identity per workload and add 
more if necessary.
...
```txt
spiffe://example.org/<region>/<dev,stage,prod>/<organization>/<workload_name>
```
...
In Vault, this is through the [TLS Certiﬁcate Auth Method][tls-cert-auth] or
[JWT/OIDC Auth Method][jwt-oidc-auth] which can be conﬁgured to recognize and 
validate JWTs and X509-SVIDs generated from SPIFFE. To enable Vault to use 
SPIFFE identities to be used, the trust bundle needs to be conﬁgured with these 
pluggable interfaces so that it can authenticate the SVIDs.

This takes care of authentication, but we still need to conﬁgure it to perform 
...
Rego policies are stored in text ﬁles so they can be centrally maintained and 
...
## 9. Comparing SPIFFE to Other Security Technologies
...
However, these existing forms of identity are not a good ﬁt for identifying 
...
from our web browsers to secure web sites. It utilizes X.509 certiﬁcates to 
...
In traditional Web PKI, certiﬁcate issuance and renewal were entirely manual 
processes. These manual processes are a poor ﬁt for modern infrastructure, where 
service instances may dynamically grow and shrink at any time. However, in the 
last few years, Web PKI has shifted to an automatic certiﬁcate issuance and 
renewal process called Domain Validation (DV).

In DV, the certiﬁcate authority sends a token to the certiﬁcate requester. The 
certificate requester shares this token using an HTTP server. The certificate 
authority accesses the token, veriﬁes it, and then signs the certiﬁcate.

The ﬁrst problem with this arrangement is that internal services frequently 
...
certificates, which would be challenging to conﬁgure. Assigning identities to 
multiple services running on one host requires separate DNS names, which would 
also be challenging to conﬁgure.

A more subtle problem is that anyone who could successfully respond to requests 
for the token could successfully acquire a certiﬁcate. This might be a different 
...
is not a good ﬁt for service identity. Many internal services that need 
...
simpler since this coordination of key material between services and a 
...
SPIRE signiﬁcantly improves security since no long-lived static credential needs 
...
information (shared secrets, typically passwords) on behalf of a workload or 
administrator. Some secrets managers can perform additional functions such as 
encrypting and decrypting data. A common feature found in many secrets managers 
is central storage, known as a *vault* , which encrypts data at rest. Workloads 
must individually authenticate to the vault before performing actions such as 
secrets retrieval or data decryption.

A typical architectural challenge in deploying secrets managers is how to 
securely store the credential used by the workload to authenticate to the 
secrets manager itself. This is sometimes called *credential zero* , the 
*bootstrap credential*, or, more broadly, the process of *secure introduction*.
...
secrets can be stored, retrieved, rotated, and revoked.

Heavy use, however, perpetuates the use of shared secrets rather than using 
strong identities.

#### How SPIFFE and SPIRE can be used to mitigate secrets managers challenges

If you do need to use a secrets manager, it can be conﬁgured to authenticate 
using SPIFFE certiﬁcates. This allows you to use the same SPIFFE certiﬁcates 
for direct authentication between services and retrieve secrets to talk to 
non-SPIFFE certificates.
...
* Mediates communication between workloads, usually through a proxy deployed 
  adjacent to each workload (side-car pattern)
...
All the major service mesh packages include a native platform-speciﬁc service 
...
partial implementations of the SPIFFE speciﬁcation (including Istio and 
...
For example, Istio uses SPIFFE for node identiﬁcation, but its identity model is 
tightly coupled to and solely based on Kubernetes speciﬁc primitives. There is 
...
Service mesh solutions that speciﬁcally implement the SPIFFE Workload API 
...
Overlay networks simulate a single uniﬁed network for services across multiple 
...
allowing them to connect. Typically, they rely on a pre-existing certiﬁcate. 
SPIFFE is a good ﬁt for providing certiﬁcates for overlay network nodes.
...
> This chapter includes ﬁve stories from practitioners who are engineers at 
...
environments using cryptographically veriﬁable identities. We started with a few 
application services and internal services, such as a workflow engine that spins 
multiple dynamic workloads to complete speciﬁc tasks by accessing data across 
...
misconﬁgurations that might result in production issues.

#### Retrofitting SPIRE into the legacy stack

SPIRE is now a key component of Uber's next infrastructure, but we are also 
using a side-car approach to retroﬁt authentication into legacy infrastructure. 
...
#### Security, development, and audit teams are beneﬁting from SPIFFE

SPIFFE gives our security team more conﬁdence in the back-end infrastructure and 
less reliance on network-based security controls. As we deal with ﬁnancial data 
...
more finely tuned identities that weren’t tied to speciﬁc hosts in our 
...
Developer velocity is signiﬁcantly better as our engineers don't have to worry 
...
were challenging as each platform-speciﬁc authentication approach had to be 
...
improve our developer productivity, forced us to build a uniﬁed identity 
...
would have required a signiﬁcant amount of effort. We went ahead with SPIRE as 
it provided the scale and ﬂexibility in supporting a wide variety of platforms 
we needed and at web-scale.

Since it offers cryptographic identity based on standard X.509 certiﬁcates, it 
...
likely to get “production errors” due to conﬁguration issues such as using 
development credentials in production.

Standardized authentication with SPIRE has also simpliﬁed compliance and audit 
...
into front-end interfaces, leveraging an ocean of de-identiﬁed health data.
...
Information (PHI) sells for much higher than ﬁnancial information; thus, 
malicious actors such as hackers and script kiddies, ﬁnd healthcare systems and 
...
breach is even higher since the threat radius increases signiﬁcantly while 
...
to cryptographic identities, such as SPIFFE. We can see additional beneﬁts of 
...
Square provides a wide variety of ﬁnancial services. Over its lifetime the 
...
Since SPIFFE was based on existing open standards such as X.509 Certiﬁcates, it 
...
added beneﬁt of a good place to bounce off system design ideas for zero trust 
systems in general.
...
* **ACLs (Access Control Lists)**: A set of rules that speciﬁes which users or 
...
* **API Token**: A general term that refers to a unique identiﬁer of a service 
...
* **Certiﬁcate Authority (CA)**: An entity that issues digital certificates.
* **Certiﬁcate Transparency**: A internet security standard and open source 
  framework for monitoring and auditing digital certiﬁcates.
...
* **Common Name (CN)**: A field in certiﬁcates generally used to specify the 
...
* **Encryption**: The process of encoding information, converting an original 
  *plain-text* into another form known as *ciphertext*, through the use of 
  algorithms to protect the conﬁdentiality of the information from unauthorized 
  parties.
...
* **JWT-SVID**: The token-based SVID in the SPIFFE speciﬁcation, aimed at 
...
* **Merkle Tree**: A hash-based tree structure in which each leaf node is a 
  ...
  Merkle trees are used in distributed systems for efficient data veriﬁcation.
...
* **Mint SVIDs**: To create a new cryptographically veriﬁable identity document.
...
* **OWASP (Open Web Application Security Project)**: A non-proﬁt foundation that 
...
* **Perimeter-based security**: A security approach that establishes a strong 
  perimeter, e.g. ﬁrewall, and then trusts activities or entities within that 
  perimeter by default.
* **PKI (Public Key Infrastructure)**: Set of policies, procedures, and tools 
  used to create, manage, and distribute digital certiﬁcates.
...
* **Scheme**: When relating to identity, a convention that deﬁnes how identity 
  can be parsed and understood.
* **Secure introduction**: The challenge of getting the ﬁrst secret from a 
...
* **Serverless**: A cloud computing execution model that allows developers to 
  ...
  allocation.
* **Service mesh**: A conﬁgurable infrastructure layer used for facilitating and 
...
* **SOX (Sarbanes-Oxley Act)**: An act passed in the United States to protect 
  ...
  need to meet.
* **SPIFFE ID**: The string that uniquely identiﬁes a workload or a service,
...
* **SAN (Subject Alternative Name)**: Alternate or additional hostnames to be 
  protected by a single SSL or certiﬁcate.
* **SVID (SPIFFE Verifiable Identity Document)**: A document with which a 
  ...
  supported formats: an X.509 certiﬁcate or a JWT token.
...
* **URI (Uniform Resource Identiﬁer)**: A string that unambiguously identiﬁes a 
...
* **Workload**: A workload is a single piece of software, deployed with a 
  particular conﬁguration for a single purpose that can consist of multiple 
  running instances, where all of them perform the same task. The term 
  *workload* may include a wide range of different deﬁnitions of a software 
...
* **X.509**: A widely used standard format for public-key certiﬁcates. These 
  certiﬁcates are used in many internet protocols, including TLS/SSL.
* **X509-SVID**: The X.509 representation of a SPIFFE Veriﬁable Identity 
  Document (SVID).
...
cryptographically veriﬁable identities for all of their services.
...
Providing secure identities to services ﬁrst requires a secure way to determine 
...
![Back cover.](assets/Image_279.jpg)
...
[riv98b]: https://people.csail.mit.edu/rivest/pubs/Riv98b.pdf "Can we eliminate certiﬁcate revocation lists?"
