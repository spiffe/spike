...
GCM technically supports multiple nonce sizes via Go's
...
2. **Make nonce size configurable** - Allow runtime or compile-time
   configuration
...
for all nonce validation. Nonce generation already correctly uses
`c.NonceSize()`
