...
- Patterns are compiled using Go's `regexp.Compile()` and matched with
  `MatchString`, which succeeds on a **substring** match
- Patterns **must** include `^` (start) and `$` (end) anchors. SPIKE compiles
  what you wrote and adds nothing; without anchors a pattern grants access
  beyond what it appears to say. See
  [ADR-0033](/architecture/adrs/adr-0033/)

**Examples:**
- ✅ Regex: `^secrets/app/.*$` (matches secrets/app/config, secrets/app/env,
  etc.)
- ✅ Regex: `^spiffe://example\.org/service$` (exact match, escaped dots)
- ❌ Not glob: `secrets/app/*` (this is NOT a valid SPIKE pattern)
- ❌ Not glob: `spiffe://example.org/service*` (this is NOT a valid SPIKE
  pattern)
