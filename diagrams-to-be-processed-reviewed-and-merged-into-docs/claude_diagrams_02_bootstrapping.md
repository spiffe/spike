...
SPIKE Bootstrap is responsible for initial system setup. It generates the root
key, creates Shamir shards, and distributes them to SPIKE Keepers.

## Bootstrap Flow (Bare-Metal and Kubernetes)
...
sequenceDiagram
    ...
    alt Kubernetes
        ...
        Lifecycle->>Lifecycle: Check environment variable<br/>SPIKE_BOOTSTRAP_COMPLETE
    end
        ...
        alt All Keepers acknowledged
            ...
            alt Hash matches
                ...
                alt Kubernetes
                    ...
                    Bootstrap->>Bootstrap: Set SPIKE_BOOTSTRAP_COMPLETE=true
                end
...
**Key Files:**
- `app/bootstrap/cmd/main.go` - Entry point
- `app/bootstrap/internal/lifecycle/lifecycle.go::ShouldBootstrap()`
- `app/bootstrap/internal/state/state.go::RootShares()`
- `app/bootstrap/internal/net/broadcast.go::BroadcastKeepers()`
- `app/bootstrap/internal/net/verify.go::VerifyInitialization()`

**Root Key Generation Details:**

```
1. Generate 32-byte cryptographically secure random seed
...
3. Create Shamir shares
   - Deterministic reader seeded with root key
...
|--------|-----------|------------|
| Bootstrap check | ConfigMap flag | Environment variable |
| Marker persistence | ConfigMap in namespace | Process environment |
...
- `SPIKE_SHAMIR_SHARES`: Total shards to generate (default: 5)
- `SPIKE_KEEPERS`: Comma-separated Keeper URLs
- `SPIKE_NEXUS_URL`: SPIKE Nexus endpoint
- `SPIKE_BOOTSTRAP_COMPLETE`: Bare-metal bootstrap flag
