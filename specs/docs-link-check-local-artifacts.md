...
- Keep the workflow output artifact and sticky PR comment behavior unchanged.
...
3. In the pull-request config, use documented lychee config syntax:
   ...
   - file-style URLs that already point inside a `docs/` tree -> local
     `docs/...`.
4. Keep retry, timeout, accepted status, report artifact, and PR comment
   behavior intact.
...
7. Validate by checking workflow syntax and confirming generated remap targets
